...
const AdminTours = () => {
  ...
  const handleDelete = (id) => {
    Swal.fire({
        ...
        text: "The data will be lost permanently!",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            .catch(() => toast.error("Cannot delete (Possibly because there is an existing order)"));
        }
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    // Validate cơ bản
    if (!formData.province_id) return toast.warn("Please enter the Province ID");
    ...
    if (editingTour) {
      ...
      axios.post('http://localhost:5000/api/tours', formData)
        .then(() => {
          toast.success("Successfully added a new tour!");
          closeModal();
          fetchTours();
        })
        .catch(err => toast.error("Error adding new!"));
    }
  };
  ...
  if (loading) return <div className="p-10 text-center text-gray-500">Loading list...</div>;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200 animate-fade-in">
      ...
      <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
        <div>
            <h2 className="font-bold text-xl text-gray-800">Tour List</h2>
            <p className="text-sm text-gray-500">Total: {tours.length} tour</p>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse min-w-[900px]">
          <thead className="bg-white text-gray-600 uppercase text-xs font-bold border-b border-gray-200">
            <tr>
              ...
              <th className="p-4">Price / Time</th>
        ...
        {tours.length === 0 && <div className="p-10 text-center text-gray-400 italic">No tour data available.</div>}
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <h3 className="text-lg font-bold text-gray-800">{editingTour ? "Update Tour" : "Add New Tour"}</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="md:col-span-2">
                            <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Tour Name</label>
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Province ID</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Price (VNĐ)</label>
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Slots</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Start Location</label>
                    ...
                    {/* Hàng 4: Mô tả */}
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">Detailed description</label>
                    ...
                    <button type="submit" className="w-full bg-red-600 text-white py-3 rounded-xl font-bold hover:bg-red-700 transition shadow-lg mt-2">
                        {editingTour ? "SAVE CHANGES" : "Create New Tour"}
                    </button>
                </form>
            </div>
          </div>
